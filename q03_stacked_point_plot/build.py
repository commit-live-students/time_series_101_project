...
def q03_stacked_point_plot(path, x_columns_name='month', y_column_name='Sales', hue='year', order_of_the_axis=['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug',
       'Sep', 'Oct', 'Nov', 'Dec']):
    
    X_train, X_valid = q02_data_splitter(path)
    X_train['month'] = X_train['Month'].dt.strftime('%b')
    X_train['year'] = X_train['Month'].dt.year
    sns.pointplot(x=x_columns_name, y= y_column_name,hue=hue, data = X_train, x_order=order_of_the_axis)
    plt.show()
    
path = 'data/perrin-freres-monthly-champagne.csv'
q03_stacked_point_plot(path)
